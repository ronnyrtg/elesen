...
namespace TradingLicense.Web.Services
{
    ...
    public class BADocumentService
    {


        public static void UpdateDocs(BannerApplicationModel bannerApplicationModel, LicenseApplicationContext ctx, int bannerApplicationId, int roleTemplate)
        {
            string[] ids = bannerApplicationModel.UploadRequiredDocids.Split(',');
            var requiredDoclist = ids.Select(x =>
            {
                var splitted = x.Split(':');
                return new RequiredDocList
            ...
            var existingRecords = new List<int>();
            var dbEntryRequiredDoc = ctx.BALinkReqDocs.Where(q => q.BannerApplicationID == bannerApplicationId).ToList();
            if (dbEntryRequiredDoc.Count > 0)
            {
                foreach (var item in dbEntryRequiredDoc)
                {
            ...
            foreach (var requiredDoc in requiredDoclist)
            {
                if (existingRecords.All(q => q != requiredDoc.RequiredDocID))
                {
                    BALinkReqDoc bALinkReqDoc = new BALinkReqDoc();
                    bALinkReqDoc.BannerApplicationID = bannerApplicationId;
                    bALinkReqDoc.RequiredDocID = requiredDoc.RequiredDocID;
                    bALinkReqDoc.AttachmentID = requiredDoc.AttachmentID;
        ...
        public static void UpdateRequiredDocs(BannerApplicationModel bannerApplicationModel, LicenseApplicationContext ctx,
            int bannerApplicationId, int roleTemplate)
        {
            var requiredDoclist = bannerApplicationModel.RequiredDocIds.ToIntList();
            ...
            foreach (var requiredDoc in requiredDoclist)
            {
                if (existingRecord.All(q => q != requiredDoc))
                {
                    BALinkReqDoc bALinkReqDoc = new BALinkReqDoc();
                    bALinkReqDoc.BannerApplicationID = bannerApplicationId;
                    bALinkReqDoc.RequiredDocID = requiredDoc;
                    ctx.BALinkReqDocs.AddOrUpdate(bALinkReqDoc);
                }
            }
            ctx.SaveChanges();
